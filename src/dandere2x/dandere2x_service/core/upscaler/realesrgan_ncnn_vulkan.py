...
class RealESRGANNCNNVulkan(AbstractUpscaler, Thread):

    def __init__(self, context: Dandere2xServiceContext, controller: Dandere2xController):
        # implementation specific
        self.active_realesrgan_subprocess = None
        self.realesrgan_vulkan_path = load_executable_paths_yaml()['realesrgan-ncnn-vulkan']

        assert get_operating_system() != "win32" or os.path.exists(self.realesrgan_vulkan_path), \
            "%s does not exist!" % self.realesrgan_vulkan_path

        super().__init__(context, controller)
        Thread.__init__(self, name="realesrgan Thread")

    # override
    def run(self, timeout=None) -> None:
        fix_w2x_ncnn_vulkan_names = Thread(target=self.__fix_realesrgan_ncnn_vulkan_names,
                                           name="RealESRGANNCNNVulkan Fix Names")
    ...
    def repeated_call(self) -> None:
        ...
        self.active_realesrgan_subprocess = subprocess.Popen(args=exec_command, shell=False,
                                                          stderr=console_output, stdout=console_output,
                                                          cwd=os.path.dirname(self.realesrgan_vulkan_path))
        self.active_realesrgan_subprocess.wait()
    ...
    def upscale_file(self, input_image: str, output_image: str) -> None:
        ...
        with open(console_output_path, "w") as console_output:
            """  
            note: 
            so realesrgan-ncnn-vulkan actually doesn't allow jpg outputs. We have to work around this by
            ...
            console_output.write(str(exec_command))
            self.active_realesrgan_subprocess = subprocess.Popen(exec_command,
    ...
    def _construct_upscale_command(self) -> list:
        realesrgan_vulkan_upscale_frame_command = [self.realesrgan_vulkan_path,
                                                "-i", "[input_file]",
                                                "-s", str(self.context.service_request.scale_factor)]

        realesrgan_vulkan_options = get_options_from_section(
            self.context.service_request.output_options["realesrgan_ncnn_vulkan"]["output_options"])

        # add custom options to realesrgan_vulkan
        for element in realesrgan_vulkan_options:
            realesrgan_vulkan_upscale_frame_command.append(element)

        realesrgan_vulkan_upscale_frame_command.extend(["-o", "[output_file]"])
        return realesrgan_vulkan_upscale_frame_command

    def __fix_realesrgan_ncnn_vulkan_names(self):
        """
        realesrgan-ncnn-vulkan will accept a file as "file.jpg" and output as "file.jpg.png".

        Unfortunately, dandere2x wouldn't recognize this, so this function renames each name to the correct naming
        convention. This function will iteratiate through every file needing to be upscaled realesrgan-ncnn-vulkan,
        ...
        - There's a really complicated try / except that exists because, even though a file may exist,
          the file handle may still be used by realesrgan-ncnn-vulkan (it hasn't released it yet). As a result,
